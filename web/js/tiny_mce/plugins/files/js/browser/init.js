...
browser.checkAgent = function() {
    if (!$.browser.version ||
        ...
        var html = '<div style="padding:10px">Su navegador no es compatible con ANIMUS FM. Por favor actualice el mismo o instale otro: <a href="http://www.mozilla.com/firefox/" target="_blank">Mozilla Firefox</a>, <a href="http://www.apple.com/safari" target="_blank">Apple Safari</a>, <a href="http://www.google.com/chrome" target="_blank">Google Chrome</a>, <a href="http://www.opera.com/browser" target="_blank">Opera</a>.';
        if ($.browser.msie)
            html += ' Usted tambien puede instalar <a href="http://www.google.com/chromeframe" target="_blank">Google Chrome Frame ActiveX plugin</a> para que internet explorer 6 ande correctamente.';
        html += '</div>';
        $('body').html(html);
        return false;
    }
...
browser.initContent = function() {
    $('div#folders').html(this.label("Cargando carpetas..."));
    $('div#files').html(this.label("Cargando archivos..."));
    $.ajax({
        type: 'GET',
        ...
        error: function(request, error) {
            $('div#folders').html(browser.label("Error desconocido."));
            $('div#files').html(browser.label("Error desconocido."));
        }
    });
};
