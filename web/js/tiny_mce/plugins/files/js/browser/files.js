...
browser.selectFile = function(file, e) {
    if (e.ctrlKey) {
        ...
        if (!files.length)
            ...
            if (files.length > 1)
                $('#fileinfo').html(files.length + ' ' + this.label("archivos seleccionados") + ' (' + size + ')');
            else {
...
browser.menuFile = function(file, e) {
    ...
    if (file.hasClass('selected') && files.length && (files.length > 1)) {
        ...
        if (this.opener.callBackMultiple) {
            html += '<a href="kcact:pick">' + this.label("Seleccione") + '</a>';
            if (thumb) html +=
                '<a href="kcact:pick_thumb">' + this.label("Miniaturas seleccionadas") + '</a>';
        ...
        html +=
            '<a href="kcact:clpbrdadd">' + this.label("Agregar al portapapeles") + '</a>' +
            '<div class="delimiter"></div>' +
            '<a href="kcact:rm"' + ((notWritable == files.length) ? ' class="denied"' : '') + '>' + this.label("Eliminar") + '</a>' +
        '</div>';
            ...
            $.each(files, function(i, cfile) {
                ...
                for (i = 0; i < browser.clipboard.length; i++)
                    if ((browser.clipboard[i].name == cdata.name) &&
                        ...
                        failed = true
                        msg += cdata.name + ": " + browser.label("Este archivo ya se encuentra en el portapapeles.") + "\n";
                        break;
                    }
        ...
        $('.menu a[href="kcact:rm"]').click(function() {
            ...
            if (failed == files.length) {
                alert(browser.label("El archivo seleccionado no puede eliminarse."))
                return false;
            }
            if (failed) {
                if (!confirm(browser.label("{count} archivos de los seleccionados no pueden eliminarse. ¿Desea eliminar el resto?", {count:failed})))
                    return false;
            } else if (!confirm(browser.label("¿Está seguro de que desea eliminar los archivos seleccionados?")))
                return false;
        ...
        if (data.thumb)
            html +='<a href="kcact:view">' + this.label("Ver") + '</a>';
        ...
        html +=
            ...
            '<a href="kcact:clpbrdadd">' + this.label("Agregar al portapapeles") + '</a>' +
            '<div class="delimiter"></div>' +
            '<a href="kcact:mv"' + (!data.writable ? ' class="denied"' : '') + '>' + this.label("Renombrar...") + '</a>' +
            '<a href="kcact:rm"' + (!data.writable ? ' class="denied"' : '') + '>' + this.label("Eliminar") + '</a>' +
        '</div>';
        ...
        $('.menu a[href="kcact:view"]').click(function() {
            browser.hideDialog();
            $('#loading').html(browser.label("Cargando imagen..."));
        ...
        $('.menu a[href="kcact:clpbrdadd"]').click(function() {
            for (i = 0; i < browser.clipboard.length; i++)
                if ((browser.clipboard[i].name == data.name) &&
                    ...
                    browser.hideDialog();
                    alert(browser.label("Este archivo ya se encuentra en el portapapeles."));
                    return false;
                }
            var cdata = data;
            cdata.dir = browser.dir;
        ...
        $('.menu a[href="kcact:mv"]').click(function(e) {
            ...
            browser.fileNameDialog(
                ...
                'newName', data.name, browser.baseGetData('rename'), {
                    ...
                    errEmpty: "Por favor ingrese el nombre del nuevo archivo.",
                    errSlash: "Caracteres no válidos en el nombre del archivo.",
                    errDot: "El archivo no puede comenzar con '.'"
                },
                function() {
                    browser.refresh();
                }
